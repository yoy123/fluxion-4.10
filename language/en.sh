...
FLUXIONSelectedBusyWITip="This is usually caused by the network manager using the interface selected. We recommend you$CGrn gracefully stop the network manager$CClr or configure it to ignored the selected interface. Alternatively, run \"export FLUXIONWIKillProcesses=1\" before fluxion to kill it but we suggest you$CRed avoid using the killer flag${CClr}."
...
FLUXIONScannerMultiInterfaceDetected="Detected multiple wireless interfaces"
FLUXIONScannerSingleInterfaceOption="Single interface scan"
FLUXIONScannerMultiInterfaceOption="Multi-interface parallel scan"
FLUXIONScannerModeQuery="Select scanning mode"
# >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
