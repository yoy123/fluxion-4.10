...
Select a proper interface make sure that it support AP mode and master mode. Both are required for fluxion. You can check both if you run the diagnostic script, located inside the scripts folder. You only have to do as root `./diagnostic [interface]`.
